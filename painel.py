...
def desenhar_menu():
    ...
    # --- TÍTULO (Usando Rule) ---
    console.print(Rule("[bold cyan]SysAdmin Helper 1.0[/bold cyan]", style="cyan"))
...
def pingar_servidor(host, nome_amigavel):
    ...
    # Comando de ping universal (Windows/Linux)
    if os.name == 'nt':
...
def limpar_fila_impressao():
    ...
    if not os.path.exists(caminho_spool):
        console.print(f"[red]ERRO: Caminho '{caminho_spool}' não encontrado.[/red]")
        return

    try:
        arquivos_na_fila = os.listdir(caminho_spool)
        if not arquivos_na_fila:
            console.print("[green]A fila de impressão já está limpa (pasta vazia).[/green]")
            return
        ...
        for arquivo in arquivos_na_fila:
            ...
            try:
                ...
                console.print(f"[dim] - Não foi possível apagar '{arquivo}' (em uso/permissão).[/dim]")
...
def gerenciar_spooler():
    ...
    while True:
        ...
        console.print("[3] Reiniciar Serviço (e limpar fila)")
        ...
        if sub_opcao == '1':
            ...
            if startup_type == 'disabled':
                ...
                os.system("net stop spooler")
                time.sleep(1) 
                limpar = console.input("Deseja também limpar a fila de trabalhos presos? (S/N): ").upper().strip()
            ...
            console.print("[bold green]Serviço definido como Automático![/bold green]")
...
while True:
    ...
    # Coloquei o mapa aqui para achar mais fácil e editar!
    DISPOSITIVOS = {
    ...
    if opcao == '1':
        ...
        caminho = console.input("\nDigite o caminho a verificar (ex: C: ou \\\\Servidor\\Pasta): ").strip()
